...
string Routine::getRoutineInfo(){
    ...
    for(int j = 0; j < exercises.size(); ++j){
        ...
        if(type_info == typeid(Running)){
            ...
            detailedContent = detailedContent +  "Calories Burned (without weight): " + to_string(p1->calculateCaloriesBurned()) + "\n";
        }
...
string Routine::getRoutineInfoWithWeight(int weight){
    string content = "-----------------------\n";
    for(int j = 0; j < exercises.size(); ++j){
        ...
        if(type_info == typeid(Running)){
            ...
            running_2 = running_2 + p1 ->getDuration(" minutes") + "\n";
            detailedContent = detailedContent + running_1 + running_2;
            detailedContent = detailedContent +  "Calories Burned (with weight): " + to_string(p1->calculateCaloriesBurned(weight)) + "\n";
        }
        else if(type_info == typeid(PushUps)){
            PushUps *p1 = (PushUps *) exercises[j];
            detailedContent = detailedContent + p1->getName() + "\n";
